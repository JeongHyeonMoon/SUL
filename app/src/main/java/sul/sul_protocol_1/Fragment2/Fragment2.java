...
public class Fragment2 extends Fragment implements OnMapReadyCallback {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState){
        ...
        View rootView = inflater.inflate(R.layout.all_friend_maps, container, false);
        ...
        // sqlite 생성
        myDb = new FriendDBHelper(this.getContext());
        ...
        if (res.getCount() == 0) {
            //친구 명수가 0이면
            Toast.makeText(this.getActivity(), "친구를 추가해주세요.", Toast.LENGTH_SHORT).show();
            //return;
        }
        ...
        if (requestCode == 1) {
            for (int i = 0; i < permissions.length; i++) {
                if (grantResults[i] == PackageManager.PERMISSION_GRANTED) {
                    ...
                    Toast.makeText(getActivity(), permissions[i] + "권한 승인 안됨", Toast.LENGTH_LONG).show();
                }
            }
        }
    }
